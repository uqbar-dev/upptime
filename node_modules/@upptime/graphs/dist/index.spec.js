...
const mockSlugify = jest.fn((value) => value
    .toLowerCase()
    .replace(/[^a-z0-9\s:/.-]/g, "")
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, ""));
...
describe("generateGraphs", () => {
    beforeEach(() => {
        jest.clearAllMocks();
        js_yaml_1.load.mockReturnValue({
            owner: "upptime",
            repo: "status",
            sites: [{ name: "Example Site" }],
        });
        fs_extra_1.readJson.mockResolvedValue([
            {
                name: "Example Site",
                slug: "example-site",
                status: "up",
    ...
    it("renders response-time graphs without point markers", async () => {
        await (0, index_1.generateGraphs)();
    ...
    it("uses Unicode site names when slugify cannot produce an ASCII slug", async () => {
        ...
        expect(fs_extra_1.ensureDir).toHaveBeenCalledWith("api/鸭鸭梨");
        expect(fs_extra_1.writeJson).toHaveBeenCalledWith("api/鸭鸭梨/uptime.json", expect.objectContaining({ message: "99.9%" }));
        expect(mockListCommits).toHaveBeenCalledWith(expect.objectContaining({ path: "history/鸭鸭梨.yml" }));
    });
});
